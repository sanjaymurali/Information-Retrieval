...
def crawler(url, keyword):
    ...
    while current_depth_urls and len(visited) < MAX_URL and depth <= MAX_DEPTH:
        ...
        if current_url not in visited:
            ...
            time.sleep(1)
            urls = parse_all_urls(current_url, keyword)
...
def crawl_url(url):
    ...
    # Takes care of redirection to same page that we have visited
...
def parse_all_urls(url, keyword):
    global KEYWORD
    ...
    try:
        ...
        if html_content != -1:
            ...
            for url in url_data:
                """
                    This regexp works by not matching 0-9 or a-z or A-Z before the word "rain" and
                    It allows Spaces and/or "-" and/or "_" and ignores the case of the word.
                """
                regexp = r'([^\w]|\b|[_-])' + re.escape(KEYWORD)
                rain_present = re.search(regexp, url.encode("UTF-8"), re.IGNORECASE)
...
def record_url(url):
    ...
    url_to_disk = open('url_list.txt', "a")
...
def start():
    delete_files()
    print("The Crawler is starting with Seed URL: " + SEED_URL + " with Keyword: " + KEYWORD)
    print("Visited pages is recorded in url_list.txt")
    print("Running...")
